...
const ListingCreate = () => {
  ...
  const loadSpaces = () => {
    ...
    fetch(SPACE_API, {
      ...
      .then((res) => res.json())
      .then((data) => {
        if (data?.data) setSpaces(() => data?.data);
      })
      .catch(console.error);
  };
  ...
  if (auth?.isAuthenticated && !spaces?.length) {
    return (
      <div className='lead text-center'>
        ...
        <Link href='/adspaces' className='btn btn-lg btn-primary'>
          create new ad space
        </Link>
      </div>
    );
                ...
                <li key={space?._id} className='card'>
                  <div className='card-head'>
                    <button
                      ...
                      Use this space to create listing
                    </button>
                  </div>
                  <div className='d-flex fw-light btn p-2'>
                    ...
                    <div className='d-flex flex-column align-items-start p-2'>
                      ...
                      <p className='d-flex w-100 justify-content-between'>
                        <span>Dimension:</span>
                        <span>
                          {space?.dimension?.width} x {space?.dimension?.height}
                          &nbsp;
                        </span>
                      ...
                      <p>{space?.address}</p>
                      <p>
                        GPS coords(DD): {space?.location?.latitude},
