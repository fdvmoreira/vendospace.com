...
const SpaceList = () => {
  ...
  const deleteSpace = (id) => {
    const SPACE_DELETE_API = `/api/v1/users/${auth?.user?._id ?? 0}/spaces/${
      id ?? 0
    }`;
  ...
  if (!adspaces?.length)
    return (
      <div className='lead d-flex flex-column align-items-center gap-2'>
        ...
        <p>
          <Link href='/adspaces/new' className='btn btn-primary bi bi-plus-lg'>
            &nbsp;create new ad space
          </Link>
        </p>
        <ToastContainer />
  ...
  return (
    <div className='container'>
      <h6 className='lead text-center'>My ad spaces</h6>
      <hr />
            ...
            <li key={space?._id}>
              {auth?.isAuthenticated && (
                <div className='d-flex bg-light justify-content-end gap-2 mb-1'>
                  <button
                    className='bi bi-pen btn btn-sm btn'
