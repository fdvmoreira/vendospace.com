  ...
  if (auth?.isAuthenticated && !auctions?.length) {
    return (
      <div className='lead text-center'>
        ...
        <Link href='/auctions/new' className='btn btn-lg btn-primary'>
          create new auction
        </Link>
      </div>
    );
            ...
            <li key={auction?._id} className='card p-2 m-3'>
              <div className='card-header'>
                <div className='d-flex justify-content-between'>
                  <span className='text-muted'>
                    Start&nbsp;{new Date(auction?.start).toLocaleDateString()}
                  </span>
                  <div>
                    <span className=''>
                      Ends:&nbsp;{new Date(auction?.end).toLocaleDateString()}
                    </span>
                    <span className=''>
                ...
                <div className='d-flex justify-content-between'>
                  <span>
                    status:{" "}
                    <span className='text-uppercase'>{auction?.status}</span>
                  </span>
                  <span>
                    Initial Price:{" "}
                    <span className='lead'>{auction?.initialPrice}</span>
                    &nbsp;&pound;
                  </span>
                </div>
              </div>
              ...
              <div className='card-footer d-flex justify-content-end gap-2'>
                <button
                  ...
                  data-bs-target='#listBidsModal'>
                  show bids
                </button>
                <button
                  className='btn btn-sm bi bi-trash btn-outline-danger'
